...
// ********* Advent of Code 2024 *********
// --- Day 13: Claw Contraption --- Puzzle 2
...
func main() {
	...
	answer := day13_2(input)
	fmt.Println(answer)

	fmt.Printf("%s \n", time.Since(start)) // время выполнения функции
}

func day13_2(machines []Machine) (result int) {

	for _, m := range machines {
		result += calcCheapestWinCombination_Cramer(m)
	}
...
func GetData(filename string) (machines []Machine) {
	...
	for scanner.Scan() {
		...
		if len(strPrize) != 0 { // это последняя строка в блоке, описывающем отдельный автомат
			...
			bx := nums[2]
			by := nums[3]
			prizeX := nums[4] + 10000000000000 // модификация координат приза по условиям задачи
			prizeY := nums[5] + 10000000000000
