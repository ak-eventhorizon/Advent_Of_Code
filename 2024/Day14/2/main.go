...
// ********* Advent of Code 2024 *********
// --- Day 14: Restroom Redoubt --- Puzzle 2
...
// Velocity: смещение на которое робот перемещается за 1 ход
...
func main() {
	...
	answer := day14_2(input)
	fmt.Println(answer)

	fmt.Printf("%s \n", time.Since(start)) // время выполнения функции
}

func day14_2(input []Robot) (result int) {
	...
	// Подход к решению задачи следующий: когда элементы складываются в фигуру - матрица имеет минимальный safetyFactor
	// Прогоняем некоторое количество симуляций, фиксируя итерацию с самым малым safetyFactor

	var iterWithMinSF int
	var minSafetyFactor int

	// 10000 итераций движения роботов на поле
	for i := range 10000 {

		currentSafetyFactor := calcSafetyFactor(*field)

		if minSafetyFactor == 0 || currentSafetyFactor < minSafetyFactor {
		...
		for k, robot := range robots {
			robot.Move()      // robot - это копия элемента слайса, для изменения самого слайса - используем следующую строку
			robots[k] = robot // изменение элемента слайса
		}
	}

	fmt.Println("Iteration =", iterWithMinSF, " SF =", minSafetyFactor)

	result = iterWithMinSF
	return result
}
